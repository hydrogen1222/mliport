"""Dependency-free JSON Schema interpreter for the validation harness.
...
deliberately not a requirement.  The committed schema documents must not
be decorative: this module executes the exact keyword subset they use and
fails loudly on any keyword it does not understand, so silently weakening
a schema is impossible.

Supported keywords
------------------
``$schema``, ``$id``, ``title``, ``description``, ``type``, ``required``,
``properties``, ``additionalProperties``, ``enum``, ``const``, ``pattern``,
``minimum``, ``maximum``, ``items``, ``default``.

Anything else raises :class:`SchemaError` when the schema is loaded.
"""
...
def _check_keywords(schema: dict[str, Any], where: str) -> None:
    ...
    if unknown:
        msg = (
            ...
            "schema_check.py deliberately instead of ignoring them"
        )
