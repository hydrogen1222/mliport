...
def test_capability_matrix_shape_and_notes():
    matrix = _matrix()
    assert matrix["schema"] == "mlipx.capability-matrix/1"
...
def test_matrix_matches_the_incar_schema_and_cli():
    from mlipx.config.schema import get_schema
    ...
    cli = (REPO / "mlipx" / "mlipx" / "cli.py").read_text(encoding="utf-8")
    assert '"--atom-map"' in cli
    assert '"--image-shifts"' in cli
    assert '"--resume"' in cli

    prepare = (REPO / "mlipx" / "mlipx" / "neb" / "prepare.py").read_text(
        encoding="utf-8"
    )
