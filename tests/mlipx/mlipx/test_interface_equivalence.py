...
calculator options). The TUI submits through the shared ``build_mlipx_command``
...
from mlipx.cli import _resolve_engine_config, create_parser
...
def _resolve_via_api(
    ...
    from mlipx.api import _api_resolve_full
...
def _resolve_via_queue(
    ...
    argv = build_mlipx_command(**kwargs)
    assert argv[:3] == [sys.executable, "-m", "mlipx.cli"], argv[:3]
...
async def test_tui_md_command_resolves_like_api_request(tmp_path: Path) -> None:
    ...
    assert command[:3] == [sys.executable, "-m", "mlipx.cli"]
