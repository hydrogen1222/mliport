...
*installed wheel*, not just the source tree. This test builds the mlipx wheel,
...
def _build_wheel(out_dir: Path) -> Path:
    source_dir = Path(__file__).resolve().parents[3] / "mlipx"
    ...
    wheels = sorted(out_dir.glob("mlipx-*.whl"))
    assert wheels, "wheel build produced no mlipx wheel"
...
def _packaged_registry_identities() -> list[dict]:
    from mlipx.capabilities import _package_evidence_dir
...
def test_installed_wheel_resolves_curated_evidence_without_evidence_dir(
    ...
    snippet = (
        "import json\n"
        "from mlipx.capabilities import _package_evidence_dir, resolve_capabilities\n"
        "out = []\n"
