...
def test_frozen_uuid_remaps_only_child(visible, flag):
    ...
    command = ["python", "-m", "mlipx", "sp", *flag]
    ...
    assert env["MLIPX_LEASED_GPU_UUID"] == "GPU-leased"
...
def test_cpu_hides_gpus_and_clears_inherited_lease():
    _, env = leased_process_environment(
        {"device": "cpu"},
        ["python"],
        {"CUDA_VISIBLE_DEVICES": "0", "MLIPX_LEASED_GPU_UUID": "GPU-old"},
    )
    assert env["CUDA_VISIBLE_DEVICES"] == ""
    assert "MLIPX_LEASED_GPU_UUID" not in env
