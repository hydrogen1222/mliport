...
from mlipx.install import inventory
from mlipx.install.plan import generate_plan
...
def test_every_resolver_stage_shares_backend_constraints():
    ...
    for engine in ("uma", "mace", "dpa", "grace"):
        ...
        assert pip
        assert all(
            s.argv[s.argv.index("--constraint") + 1] == f"{venv}/mlipx-constraints.txt"
            for s in pip
        )
        assert any(
...
def test_inventory_rejects_changed_environment(tmp_path, monkeypatch):
    current = {
        "schema": "mlipx.install-inventory/1",
        "python": "3.12.0",
