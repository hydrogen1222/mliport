...
def test_read_only_bootstrap_missing_runtime_never_downloads(tmp_path, flag):
    ...
    result = subprocess.run(
        ["bash", "scripts/install_mliport.sh", flag],
...
def test_bootstrap_preserves_argv_and_runtime_path_with_spaces(tmp_path):
    ...
    result = subprocess.run(
        ["bash", "scripts/install_mliport.sh", *args],
        cwd=Path(__file__).resolve().parents[3],
        env=env,
        capture_output=True,
        text=True,
    ...
    assert json.loads(calls.read_text()) == ["-m", "mliport.install", *args]
...
def test_offline_bootstrap_never_downloads_python(
    ...
    result = subprocess.run(
        ["bash", "scripts/install_mliport.sh", *source_args, "--dry-run"],
        cwd=Path(__file__).resolve().parents[3],
        env=env,
        capture_output=True,
        text=True,
...
def test_invalid_target_python_fails_before_uv_download(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        ["bash", "scripts/install_mliport.sh", "--python", "3.9", "--dry-run"],
        cwd=Path(__file__).resolve().parents[3],
        env=env,
        capture_output=True,
        text=True,
...
def test_online_explicit_planner_override_skips_target_lookup(tmp_path: Path) -> None:
    """MLIPORT_INSTALL_PYTHON plans the install; the target env is built later by uv."""
    calls = tmp_path / "uv-calls.txt"
    ...
    fake_uv.chmod(0o755)
    env = os.environ.copy()
    env["MLIPORT_INSTALL_PYTHON"] = sys.executable
    env["PATH"] = f"{tmp_path}:{env['PATH']}"
