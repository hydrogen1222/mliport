...
``mliport.api`` exports, ``pyproject.toml``, the beta summary and the
...
if str(REPO_ROOT / "mliport") not in sys.path:
    sys.path.insert(0, str(REPO_ROOT / "mliport"))
...
PYPROJECT = REPO_ROOT / "mliport" / "pyproject.toml"
...
def _top_level_choices() -> dict[str, Any]:
    from mliport.cli import create_parser  # noqa: PLC0415
...
def test_readme_api_names_exist() -> None:
    import mliport.api  # noqa: PLC0415
    ...
    missing = documented - set(mliport.api.__all__)
