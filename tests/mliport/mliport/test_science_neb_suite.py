...
``mliport.neb.workflow.run_neb_workflow`` path; these tests exercise the product
...
def test_barrier_metrics_semantics_and_no_invented_saddle():
    from mliport.neb.results import barrier_metrics
...
def _write_band_checkpoint(tmp_path, images):
    ...
    return store.write(
        ...
        stage="ci_neb",
        stage_step=1,
        resume_fingerprint={"schema": "mliport.neb-resume-fingerprint/1"},
...
def test_resume_fingerprint_covers_full_option_set():
    ...
    from mliport.neb.workflow import _fingerprint
    ...
    assert fp_a["schema"] == "mliport.neb-resume-fingerprint/1"
...
def test_checkpoint_store_roundtrip_bitwise(tmp_path):
    ...
    from mliport.neb.io import NEBCheckpointStore, load_checkpoint
    from mliport.neb.prepare import prepare_band
    from mliport.neb.schema import NEBOptions
    ...
    fingerprint = {"schema": "mliport.neb-resume-fingerprint/1", "probe": True}
    path = store.write(
        band.images,
        run_id="run-a",
