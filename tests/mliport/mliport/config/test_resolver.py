"""Tests for mliport.config.resolver (layered config resolution)."""
...
def test_model_alias_path_uses_declaring_file_not_highest_priority_file(
    ...
    monkeypatch.setenv("MLIPORT_SETTINGS", str(user_settings))
...
def test_incar_types_and_relative_model_path_follow_schema(tmp_path: Path) -> None:
    incar_path = tmp_path / "input" / "INCAR.mliport"
