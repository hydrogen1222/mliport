"""Tests for mliport.config.settings (settings.ini loading)."""
...
def test_search_paths_includes_user_config() -> None:
    paths = settings_search_paths()
    user = Path.home() / ".config" / "mliport" / "settings.ini"
    assert user in [
        p.resolve() if p.is_absolute() else Path.cwd() / p for p in paths
    ] or any("mliport/settings.ini" in str(p) for p in paths)
...
def test_settings_record_each_value_declaring_file(
    ...
    monkeypatch.setenv("MLIPORT_SETTINGS", str(low))
...
def test_load_settings_env_var() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        ini.write_text("[general]\nlog_level = DEBUG\n")
        old = os.environ.get("MLIPORT_SETTINGS")
        os.environ["MLIPORT_SETTINGS"] = str(ini)
        try:
            s = load_settings()
            assert ini in s.loaded_paths
            ...
            if old is not None:
                os.environ["MLIPORT_SETTINGS"] = old
            else:
                os.environ.pop("MLIPORT_SETTINGS", None)
...
def test_parse_model_alias_in_resolver() -> None:
    """Aliases are resolved via resolve_config, not stored on MliportSettings."""
    with tempfile.TemporaryDirectory() as d:
...
def test_explicit_beats_env() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        old = os.environ.get("MLIPORT_SETTINGS")
        os.environ["MLIPORT_SETTINGS"] = str(env_ini)
        try:
...
def test_init_settings_file_project() -> None:
    with tempfile.TemporaryDirectory() as d:
        ...
        assert "mliport" in Path(path).read_text().lower()
