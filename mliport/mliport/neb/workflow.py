...
from mliport.neb.prepare import BandInput, prepare_band
from mliport.neb.revisions import (
...
if TYPE_CHECKING:
    ...
    from mliport.config.resolver import ResolvedConfig
...
def _fingerprint(
    ...
    *,
    ...
    if package_version is None:
        raise NEBPreparationError(
            "Cannot determine mliport version for resume identity"
        )
    value = {
        "schema": "mliport.neb-resume-fingerprint/1",
        "software": {
            "mliport_version": package_version,
...
def _run_neb_workflow_locked(
    ...
    *,
    ...
    if resolved.device != "cpu":
        from mliport.validation import evaluate
    ...
    from mliport.capabilities import require_neb_capability, resolve_capabilities
    ...
    if device_uuid is not None:
        from mliport.devices import verify_runtime_uuid
    ...
    if mliport_version is None:
        raise NEBPreparationError(
            "Cannot determine the exact mliport package version; install mliport "
            "before starting a provenance-bearing NEB run"
        )
    context = {
        "schema": "mliport.neb-run-context/1",
        "run_id": run_id,
        "attempt_id": attempt_id,
        "started_at": _utc_now(),
        "geometry_resume_only": True,
