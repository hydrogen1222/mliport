...
from mliport.analysis.validation import InvalidTrajectoryError, UnsupportedAnalysisError
...
def _update_index(root: Path, analysis_id: str, record: dict[str, Any]) -> None:
    ...
    with _index_lock(root):
        try:
            ...
            index = {"schema": "mliport.analysis-index/2", "jobs": {}}
...
def _publish_attempt(
    ...
    _write_json(
        ...
        {
            "schema": "mliport.analysis-attempt-pointer/1",
...
def _write_columns(path: Path, columns: dict[str, Any]) -> None:
    ...
    if extra:
        manifest = {
            "schema": "mliport.analysis-array-manifest/1",
            "source": path.name,
            "arrays": {
...
def _dispatch(request: AnalysisRequest, output_dir: Path) -> tuple[Any, list[str]]:
    ...
    if task == "validate":
        from mliport.analysis.validation import validate_trajectory

        return validate_trajectory(dataset).to_dict(), artifacts
    if task == "thermo":
        from mliport.analysis.plots import plot_thermo
        ...
        return result, artifacts
    if task == "rdf":
        from mliport.analysis.plots import plot_rdf
        from mliport.analysis.structure import radial_distribution
        ...
        return result, artifacts
    if task == "rmsd":
        from mliport.analysis.structure import periodic_rmsd_rmsf
    ...
    if task == "msd":
        ...
        from mliport.analysis.plots import plot_msd, plot_msd_alpha
        ...
        return result, artifacts
    if task == "density":
        from mliport.analysis.structure import density_map
        ...
        return result, artifacts
    if task == "transport":
        from mliport.analysis.plots import (
            plot_transport,
            plot_transport_mscd,
            plot_transport_mstd,
        )
    ...
    if task in {"vacf", "spectrum"}:
        from mliport.analysis.plots import plot_spectrum, plot_vacf
...
def run_analysis(request: AnalysisRequest) -> dict[str, Any]:
    ...
    canonical_request = {
        "schema": "mliport.analysis-request/2",
        "task": request.task,
    ...
    provenance = {
        "schema": "mliport.analysis-provenance/2",
        "analysis_id": analysis_id,
    ...
    try:
        ...
        payload = {
            "schema": "mliport.analysis-results/2",
            "analysis_id": analysis_id,
            "attempt_id": attempt_id,
            "status": "success",
            "task": request.task,
        ...
        _publish_attempt(
            ...
            {
                "attempt.json": {
                    "schema": "mliport.analysis-attempt/1",
                    "attempt_id": attempt_id,
                    "analysis_id": analysis_id,
                    "task": request.task,
                    "status": "success",
...
def _publish_failed_attempt(
    ...
    *,
    ...
    finished_at = datetime.now(timezone.utc).isoformat()
    error = {
        "schema": "mliport.analysis-error/2",
        "analysis_id": analysis_id,
        "attempt_id": attempt_id,
        "status": status,
