...
def _require_kinisi():
    try:
        ...
        raise OptionalDependencyError(
            "kinisi transport is unavailable. Install mliport[transport]."
        ) from exc
    try:
    ...
    if lag_step_ps is None and lag_stop_ps is None:
        ...
        if estimated_native_lag_points > native_lag_guard or (
            ...
            raise ValueError(
                ...
                "\n\n"
                "This is too large for mliport's guarded transport path "
...
class _KinisiFrames(NamedTuple):
    ...
    exact drift-corrected displacement history that mliport validated.  The
...
def _exact_displacement_parser(
    *,
    ...
    """Build a kinisi parser that consumes mliport's exact displacements.
    ...
    continuous positions mliport already validated.
    """
    ...
    if used.shape != reference.shape:
        raise UnsupportedAnalysisError(
            "kinisi displacement-array shape changed: the exact mliport "
    ...
    if max_difference > _KINISI_RECONSTRUCTION_ATOL_A:
        raise UnsupportedAnalysisError(
            "kinisi is not consuming the exact mliport displacements; refusing "
    ...
    parser._mliport_exact_displacement_audit = {
        "backend_displacements_source": "mliport exact continuous positions",
...
def _build_kinisi_analyzer(
    *,
    ...
    if ASEParser is not None:
        ...
        audit = dict(parser._mliport_exact_displacement_audit)
...
def _validate_kinisi_periodic_reconstruction(
    ...
    mliport validates the continuous positions it will inject into the kinisi
    ...
    if convention == "unwrapped":
        ...
        return {
            ...
            "backend_reconstruction": "mliport exact continuous displacement array",
    ...
    return {
        ...
        "backend_input": (
            "wrapped frames plus mliport MIC-reconstructed continuous positions"
        ),
        "backend_input_frames_wrapped_for_kinisi": True,
        "backend_reconstruction": "mliport MIC reconstruction of wrapped frames",
    ...
    result: dict[str, Any] = {
        ...
        "drift_correction": {
            **drift_semantics(
            ...
            "backend_semantics": (
                "mliport pre-applies the explicitly recorded definition once and "
