...
* a per-key ``source`` trace so ``mliport config explain TEMPERATURE`` can say
...
from mliport.config.schema import Schema, get_schema

if TYPE_CHECKING:
    from typing import Any

    from mliport.config.aliases import ModelAlias, Profile
    from mliport.config.settings import MliportSettings
...
def resolve_config(
    *,
    calc_type: str,
    settings: MliportSettings | None = None,
    ...
    Args:
        ...
        settings: Loaded :class:`MliportSettings` (may be None).
...
def _aliases_from_settings(settings: MliportSettings | None) -> dict[str, ModelAlias]:
    if settings is None:
        return {}
...
def _profiles_from_settings(settings: MliportSettings | None) -> dict[str, Profile]:
    if settings is None:
        return {}
    from mliport.config.aliases import parse_profiles  # noqa: PLC0415
